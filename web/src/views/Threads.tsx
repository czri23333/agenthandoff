  ...
  return (
    <div className="flex h-full flex-col">
      <div className="flex items-center gap-3 border-b border-zinc-800/70 px-5 py-2.5">
        <p className="text-[12px] text-zinc-500">{t("threadsDesc")}</p>
        <div className="ml-auto flex items-center gap-2">
          <label className="text-[11px] text-zinc-500">{t("minOverlap")} {minOverlap.toFixed(2)}</label>
          ...
          <button onClick={recluster} className="rounded-md border border-zinc-700 bg-zinc-800/60 px-2 py-1 text-[11px] text-zinc-300 hover:bg-zinc-700">{t("recluster")}</button>
        </div>
      </div>
      <div className="min-h-0 flex-1 overflow-y-auto p-4">
        {threads === null && <p className="text-[13px] text-zinc-600">{t("clustering")}</p>}
        {multi?.length === 0 && <p className="text-[13px] text-zinc-600">{t("noThreads")}</p>}
        {singleCount > 0 && (
          <p className="mb-3 text-[11px] text-zinc-600">{singleCount} {t("standaloneHidden")}</p>
        )}
        <div className="grid grid-cols-1 gap-3 xl:grid-cols-2">
          {multi?.map((th, i) => (
            <section key={i} className="rounded-xl border border-zinc-800 bg-zinc-900/50 p-3">
              <header className="mb-2 flex items-center gap-2">
                <span className="rounded bg-zinc-800 px-1.5 py-px font-mono text-[10px] text-zinc-400">{th.session_ids.length} {t("sessionsN")}</span>
                <span className="font-mono text-[10px] text-zinc-600">{th.clis.join(" + ")}</span>
                <span className="ml-auto font-mono text-[10px] text-zinc-600">{th.last_active?.slice(0, 10)}</span>
              </header>
              <pre className="whitespace-pre-wrap font-mono text-[11px] leading-[1.7] text-zinc-400">{th.lines.slice(1).join("\n")}</pre>
            </section>
          ))}
        </div>
      </div>
