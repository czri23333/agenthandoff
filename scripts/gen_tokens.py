...
COMPONENTS: dict[str, dict] = {
    ...
    "navigationRail": {
        "_source": (
            "androidx tokens: NavigationRailCollapsedTokens.kt (96dp container width, "
            "80dp narrow width, 4dp item vertical space, 44dp top space, corner-none "
            "container, level0 elevation, surface container) and "
            "NavigationRailVerticalItemTokens.kt (32\u00d756dp active indicator, 4dp "
            "icon-label space, 16dp leading/trailing space, label-medium). The 24dp icon "
            "and the corner-full active-indicator shape come from "
            "NavigationRailBaselineItemTokens.kt; its 6dp ContainerVerticalSpace belongs "
            "to the baseline item variant, which this rail does not render. Colours from "
            "NavigationRailColorTokens.kt: active indicator secondary-container, active "
            "icon on-secondary-container, active label secondary, inactive icon/label "
            "on-surface-variant, and ItemActive/InactiveHovered+PressedStateLayer all "
            "on-secondary-container. "
            "on-surface-variant. Arrangement from androidx NavigationRail.kt: "
            "widthIn(min = narrow), 4dp vertical padding, spacedBy(4dp), item 80\u00d756dp, "
            "indicator = icon + 4dp around it."
        ),
        "container": {
            "width": 96,
            "narrowWidth": 80,
        ...
        "typeRole": "@type:label-medium",
        "active": {
            "indicator": "@role:secondary-container",
            "icon": "@role:on-secondary-container",
            "label": "@role:secondary",
        },
        "inactive": {
            "icon": "@role:on-surface-variant",
            "label": "@role:on-surface-variant",
        },
        # NavigationRailColorTokens.kt publishes six state-layer colours and all
        # six are the same role, so the two the sheet spends are named once and
        # the file is cited for the rest.
        "stateLayer": {
            "hovered": "@role:on-secondary-container",
