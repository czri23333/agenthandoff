...
| 7 | Per-message billing | None in message area (confirms billing-coverage §5-16); usage = header waterlevel hover | renderer `@17698884/@17702769`; projection `Et()` | MATCH. The `dur_ms` chip that exceeded it is gone (m3components slice): the measured duration now rides the row's hover tip and the model chip's `title`, which is where the rule puts usage. Measured after (re-measured 2026-09-12 on the built bundle, session `sess_6c83d699`): **0** `⏱` glyphs anywhere in the rendered page, and **209** elements carrying the measured duration in their `title` (`2026/9/12 14:33:26 · `+14.0s`, `+20.0s`, `+712.0s`), beside 254 timestamp tips. The per-message *token* counts stay — §5-16 rules on what the official can show, and our stores record usage the official's client cannot see |
