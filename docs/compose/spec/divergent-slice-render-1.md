...
**Journey log** — Baseline measurement rendered blank (0 nodes) and exposed the MIME boot failure; without measuring first this ships-broken-on-some-machines bug stays hidden. `uv run` re-syncs undeclared packages away: measurement-only `playwright` must be used via direct venv python, never `uv run`. Readable-but-empty snapshot flagging transcripts is literally true ("no longer listed"), not overclaim — the honesty check that shaped the badge. `index.html` line-ending churn in rebuilds is cosmetic; leave it.
