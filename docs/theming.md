...
| M3E springs | `SPRINGS` | 2 schemes × 6 roles, each a damping/stiffness pair | `ExpressiveMotionTokens.kt`, `StandardMotionTokens.kt` and `MotionScheme.kt` from **androidx material3 v0_14_0** — 12 pairs, **0 differences**, and each scheme is asserted to reference only its own token file |
...
The springs come from a **third repository** — androidx material3 v0_14_0 —
because material-web does not publish them at all. `MotionScheme.kt` is what makes
the *names* checkable rather than assumed: it has one impl per scheme, six specs
each, and the test asserts that the standard impl references only
`StandardMotionTokens` and the expressive impl only `ExpressiveMotionTokens`, so
our `standard-*` and `expressive-*` prefixes cannot be swapped. What stays ours is
the step after the pair: CSS has no spring primitive, so each pair is **sampled**
into a `linear()` easing (24 samples, settle tolerance 0.001) with a cubic-bezier
fallback, and that sampling is a conversion rather than a published value —
`docs/theming.md`'s deviation register says so, and the token file carries both
the sampled curve and the fallback.
...
| a card header carries a title and one trailing affordance | the session-detail side pane's brief card carries the title and the 摘要/全文 switch in the header, and its three transport actions moved into the body | the side pane is `PaneScaffoldDirective`'s preferred width — 412px at Large, 360px at Expanded; the title row plus three buttons came to ~450px, and antd's single-row head painted them over each other — measured: the `.zip` button at x 1081–1187 over the switch at x 1109–1213, and over the title. M3 publishes no "card header with a toolbar", and a second row that wraps is the honest answer at this width. The gate that finds this class of defect (`--overlap`) compares every pair of rendered controls on every route, in both themes, and on a session-detail route |
...
| `NavigationRailCollapsedTokens.ContainerWidth` (96dp) and `TopSpace` (44dp) | carried as the rail landmark's `max-width` and `scroll-margin-block-start` | the collapsed rail *is* the narrow form: `NavigationRail.kt` sets `widthIn(min = NarrowContainerWidth)` (80dp) and pads 4dp, so 96 and 44 describe a rail this shell does not render. They are spent where they are still true (a cap and scroll clearance) and the reason sits beside them rather than the values being dropped |
| `PaneScaffoldDirective.DefaultPreferredHeight` (420dp) | not spent | it sizes a **vertical** partition — a tabletop split — and the web shell has none: its panes fill the column. `tests/test_official_panes.py` reads the value from the vendored file so it cannot drift while unused |
...
One animation had a contract and no witness, which is a worse state than having
no contract: the loading indicator. `m3.css` says the container turns once per
`--ah-motion-duration-extra-long4` on the linear easing and that its `::after`
shape morphs at a quarter of that period — and nothing checked either claim, so a
static dot would have passed every gate in this file. `--loading` reads both
animations off the Web Animations API *while they run* and samples the shape's
computed `border-radius` half a morph period apart, so a shape that does not move
is a failure rather than a silent no-op. Measured: **1000ms** turn, **250ms**
morph, `Infinity` iterations, and a different radius at the two samples in all
four indicator/theme readings. `LoadingIndicatorTokens` publishes no motion of its
own, so the pair is the system duration tokens rather than a component's, and on a
`%TEMP%` copy whose morph runs at the full period the gate exits 1 with
`the morph period is 1000ms, not a quarter of the 1000ms turn`.
